...
class DenseCorrespondenceTraining(object):
    ...
    def run(self, loss_current_iteration=0, use_pretrained=False):
        """
        Runs the training
        loss_type can be 'distributional' or 'contrastive'
        """
        ...
        loss_type = self._config['dense_correspondence_network']['loss_type']

        if loss_type == 'contrastive':
            pixelwise_loss = PixelwiseContrastiveLoss(image_shape=dcn.image_shape, config=self._config['loss_function'])
        elif loss_type == 'distributional':
            pixelwise_loss = PixelwiseDistributionalLoss(image_shape=dcn.image_shape, config=self._config['loss_function'])
        else:
            raise ValueError("Unknown loss type, should be either 'contrastive' or 'distributional'")
        
        pixelwise_loss.debug = True
        ...
        for epoch in range(50):  # loop over the dataset multiple times
            for i, data in enumerate(self._data_loader, 0):
                ...
                # transform variables in proper format and move them to cuda
                ...
                if loss_type == 'contrastive':
                    loss, match_loss, masked_non_match_loss, background_non_match_loss, blind_non_match_loss = \
                        loss_composer.get_contrastive_loss(pixelwise_loss, match_type,
                                                           img_a, img_b,
                                                           ...
                                                           background_non_matches_a, background_non_matches_b,
                                                           blind_non_matches_a, blind_non_matches_b,
                                                           symmetry=self._config['dense_correspondence_network']['symmetry'])
                elif loss_type == 'distributional':
                    loss = loss_composer.get_distributional_loss(pixelwise_loss,
                                                                 img_a, img_b,
                                                                 image_a_pred, image_b_pred,
                                                                 matches_a, matches_b,
                                                                 img_a_mask, img_b_mask,
                                                                 sigma=self._config['dense_correspondence_network']['sigma'],
                                                                 symmetry=self._config['dense_correspondence_network']['symmetry'])
                loss.backward()
                optimizer.step()
                ...
                def update_plots(loss, match_loss, masked_non_match_loss, background_non_match_loss,
                    ...
                    self._logging_dict['train']['total_loss'].append(loss.item())

                    self._tensorboard_logger.log_value("train masked non match loss", masked_non_match_loss.item(),
                                                       loss_current_iteration)

                    self._logging_dict['train']['background_non_match_loss'].append(background_non_match_loss.item())
                    self._tensorboard_logger.log_value("train background non match loss",
                                                       background_non_match_loss.item(), loss_current_iteration)

                    if data_type == SpartanDatasetDataType.SINGLE_OBJECT_WITHIN_SCENE:
                        self._tensorboard_logger.log_value("train blind SINGLE_OBJECT_WITHIN_SCENE",
                                                           blind_non_match_loss.item(), loss_current_iteration)

                    if data_type == SpartanDatasetDataType.DIFFERENT_OBJECT:
                        self._tensorboard_logger.log_value("train blind DIFFERENT_OBJECT", blind_non_match_loss.item(),
                                                           loss_current_iteration)

                    # loss is never zero
                    if data_type == SpartanDatasetDataType.SINGLE_OBJECT_WITHIN_SCENE:
                        self._tensorboard_logger.log_value("train loss SINGLE_OBJECT_WITHIN_SCENE", loss.item(),
                                                           loss_current_iteration)

                    elif data_type == SpartanDatasetDataType.DIFFERENT_OBJECT:
                        self._tensorboard_logger.log_value("train loss DIFFERENT_OBJECT", loss.item(),
                                                           loss_current_iteration)

                    elif data_type == SpartanDatasetDataType.SINGLE_OBJECT_ACROSS_SCENE:
                        self._tensorboard_logger.log_value("train loss SINGLE_OBJECT_ACROSS_SCENE", loss.item(),
                                                           loss_current_iteration)

                    elif data_type == SpartanDatasetDataType.MULTI_OBJECT:
                        self._tensorboard_logger.log_value("train loss MULTI_OBJECT", loss.item(),
                                                           loss_current_iteration)

                    elif data_type == SpartanDatasetDataType.SYNTHETIC_MULTI_OBJECT:
                        self._tensorboard_logger.log_value("train loss SYNTHETIC_MULTI_OBJECT", loss.item(),
                                                           loss_current_iteration)
                    else:
                        raise ValueError("unknown data type")

                    if data_type == SpartanDatasetDataType.DIFFERENT_OBJECT:
                        self._tensorboard_logger.log_value("train different object", loss.item(),
                                                           loss_current_iteration)
                ...
                if self._config["training"]["compute_test_loss"] and (loss_current_iteration % compute_test_loss_rate == 0) and loss_current_iteration > 5:
                    ...
                    test_loss, test_match_loss, test_non_match_loss = \
                        DCE.compute_loss_on_dataset(dcn,
    ...
    def setup_logging_dir(self):
        ...
        if 'logging_dir_name' in self._config['training']:
            ...
            dir_name = utils.get_current_time_unique_name() + "_" + str(
                self._config['dense_correspondence_network']['descriptor_dimension']) + "d"

        self._logging_dir_name = dir_name

        self._logging_dir = os.path.join(utils.convert_to_absolute_path(self._config['training']['logging_dir']),
                                         dir_name)
    ...
    def save_network(self, dcn, optimizer, iteration, logging_dict=None):
        ...
        if logging_dict is not None:
            log_history_file = os.path.join(self._logging_dir,
                                            utils.getPaddedString(iteration, width=6) + "_log_history.yaml")
