...
class SpartanDataset(DenseCorrespondenceDataset):
    ...
    def __init__(self, debug=False, mode="train", config=None, config_expanded=None):
        ...
        :param config: This is for creating a dataset from a composite dataset config file.
            This is of the form:
                logs_root_path: /home/aportillo/pytorch-dense-correspondence/pdc/logs_proto
        ...
        if self.debug:
            ...
            self.num_background_non_matches_per_match = 10
            self.num_masked_non_matches_per_match = 10
    ...
    def get_image_filename(self, scene_name, img_index, image_type):
        ...
        if image_type == ImageType.RGB:
            ...
            file_extension = "rgb-%s.png"
        elif image_type == ImageType.DEPTH:
            images_dir = os.path.join(scene_directory, 'depth_values')
            file_extension = "depth-%s.exr"
        elif image_type == ImageType.MASK:
            images_dir = os.path.join(scene_directory, 'image_masks')
            file_extension = "mask-%s.png"
        else:
        ...
        return os.path.join(images_dir, file_extension % img_index)
    
    def get_height_and_width(self):
        scene_name = self.get_random_scene_name()
        image_idx = self.get_random_image_index(scene_name)
        image_rgb, _ = self.get_rgb_mask(scene_name, image_idx)
        height, width, _ = np.array(image_rgb).shape
        return height, width
    ...
    def get_random_image_index(self, scene_name):
        ...
        if self._image_index_sample_range is None:
            ...
            image_idxs = list(knots_info.keys())
    ...
    def get_within_scene_data(self, scene_name, metadata, canonical_comparison=True):
        """
        ...
        img_a_knots = self._knots_info[scene_name][str(image_a_idx)]
        
        if canonical_comparison:
            image_b_idx = self.get_random_image_index('shirt_canonical')
            image_b_rgb, image_b_mask = self.get_rgb_mask('shirt_canonical', image_b_idx)
            img_b_knots = self._knots_info['shirt_canonical'][str(image_b_idx)]
        else:
            image_b_idx = self.get_random_image_index(scene_name)
            image_b_rgb, image_b_mask = self.get_rgb_mask(scene_name, image_b_idx)
            img_b_knots = self._knots_info[scene_name][str(image_b_idx)]
        metadata['image_b_idx'] = image_b_idx

        # find correspondences
        uv_a, uv_b = correspondence_finder.batch_find_pixel_correspondences(img_a_knots, img_b_knots)

        # data augmentation
        # TODO (ASP): data augmentation
        ...
        image_height, image_width, _ = np.array(image_a_rgb).shape
        image_b_shape = (image_height, image_width)
        

        uv_b_masked_non_matches = \
            correspondence_finder.create_non_correspondences(uv_b, image_b_shape,
        ...
        image_a_mask = np.array(image_a_mask)
        image_b_mask = np.array(image_b_mask)

        matches_a = SD.flatten_uv_tensor(uv_a, image_width, image_height)
        matches_b = SD.flatten_uv_tensor(uv_b, image_width, image_height)

        # Masked non-matches
        uv_a_masked_long, uv_b_masked_non_matches_long = \
            self.create_non_matches(uv_a, uv_b_masked_non_matches, self.num_masked_non_matches_per_match)

        masked_non_matches_a = SD.flatten_uv_tensor(uv_a_masked_long, image_width, image_height).squeeze(1)
        masked_non_matches_b = SD.flatten_uv_tensor(uv_b_masked_non_matches_long, image_width, image_height).squeeze(1)

        # Non-masked non-matches
        uv_a_background_long, uv_b_background_non_matches_long = \
            self.create_non_matches(uv_a, uv_b_background_non_matches, self.num_background_non_matches_per_match)

        background_non_matches_a = SD.flatten_uv_tensor(uv_a_background_long, image_width, image_height).squeeze(1)
        background_non_matches_b = SD.flatten_uv_tensor(uv_b_background_non_matches_long, image_width, image_height).squeeze(1)
        ...
        if len(blind_non_matches_a) == 0:
            ...
            if num_blind_samples > 0:
                ...
                blind_uv_b = correspondence_finder.random_sample_from_masked_image_torch(image_b_mask_torch,
                                                                                         num_blind_samples)
        ...
        if self.debug:
            ...
            plot_uv_a_masked_long, plot_uv_b_masked_non_matches_long = SD.subsample_tuple_pair(uv_a_masked_long,
            ...
            plot_blind_uv_a, plot_blind_uv_b = SD.subsample_tuple_pair(blind_uv_a, blind_uv_b,
                                                                       num_samples=num_matches_to_plot * 10)
        ...
        if self.debug:
            ...
            if uv_a is not None:
                ...
                plt.imshow(np.array(image_a_mask))
                plt.title("Mask of img a object pixels")
                plt.show()

                plt.imshow(np.array(image_a_mask) - 1)
        ...
        return metadata["type"], image_a_rgb, image_b_rgb, image_a_mask, image_b_mask, matches_a, matches_b, \
               masked_non_matches_a, masked_non_matches_b, background_non_matches_a, background_non_matches_b, \
    ...
    def get_synthetic_multi_object_within_scene_data(self):
        ...
        matches_a = SD.flatten_uv_tensor(matches_1, image_width, image_height)
        matches_b = SD.flatten_uv_tensor(matches_2, image_width, image_height)

        # Masked non-matches
        uv_a_masked_long, uv_b_masked_non_matches_long = self.create_non_matches(matches_1,
                                                                                 matches_2_masked_non_matches,
        ...
        # Non-masked non-matches
        uv_a_background_long, uv_b_background_non_matches_long = self.create_non_matches(matches_1,
        ...
        if self.debug:
            ...
            # correspondence_plotter.plot_correspondences_direct(image_a1_rgb, np.array(image_a1_depth),
            #                                                        image_a2_rgb, np.array(image_a2_depth),
            ...
            # correspondence_plotter.plot_correspondences_direct(image_b1_rgb, np.array(image_b1_depth),
            #                                                        image_b2_rgb, np.array(image_b2_depth),
            #                                                        plot_uv_b1, plot_uv_b2,
            #                                                        circ_color='g', show=True)

            print("MERGED")
            plot_uv_1, plot_uv_2 = SpartanDataset.subsample_tuple_pair(matches_1, matches_2,
                                                                       num_samples=num_matches_to_plot)
            plot_uv_a_masked_long, plot_uv_b_masked_non_matches_long = \
                SpartanDataset.subsample_tuple_pair(uv_a_masked_long, uv_b_masked_non_matches_long,
                                                    num_samples=num_matches_to_plot)

            plot_uv_a_background_long, plot_uv_b_background_non_matches_long = \
                SpartanDataset.subsample_tuple_pair(uv_a_background_long, uv_b_background_non_matches_long,
                                                    num_samples=num_matches_to_plot)

            fig, axes = correspondence_plotter.plot_correspondences_direct(merged_rgb_1_PIL, np.array(image_b1_depth),
                                                                           merged_rgb_2_PIL, np.array(image_b2_depth),
                                                                           plot_uv_1, plot_uv_2,
                                                                           circ_color='g', show=False)

            correspondence_plotter.plot_correspondences_direct(merged_rgb_1_PIL, np.array(image_b1_depth),
                                                               merged_rgb_2_PIL, np.array(image_b2_depth),
                                                               plot_uv_a_masked_long, plot_uv_b_masked_non_matches_long,
                                                               use_previous_plot=(fig, axes),
        ...
        return metadata[
                   "type"], merged_rgb_1, merged_rgb_2, matches_a, matches_b, masked_non_matches_a, masked_non_matches_b, background_non_matches_a, background_non_matches_b, SD.empty_tensor(), SD.empty_tensor(), metadata
    ...
    def get_across_scene_data(self, scene_name_a, scene_name_b, metadata):
        ...
        blind_uv_a = correspondence_finder.random_sample_from_masked_image_torch(np.array(image_a_mask), num_samples)
        # sample random indices from mask in image b
        blind_uv_b = correspondence_finder.random_sample_from_masked_image_torch(np.array(image_b_mask), num_samples)
        ...
        if not self.debug:
            ...
            [image_a_rgb, image_a_depth,
             image_a_mask], blind_uv_a = correspondence_augmentation.random_image_and_indices_mutation(
                [image_a_rgb, image_a_depth, image_a_mask], blind_uv_a)
            [image_b_rgb, image_b_depth,
             image_b_mask], blind_uv_b = correspondence_augmentation.random_image_and_indices_mutation(
                [image_b_rgb, image_b_depth, image_b_mask], blind_uv_b)

        image_a_depth_numpy = np.array(image_a_depth)
        image_b_depth_numpy = np.array(image_b_depth)

        image_b_shape = image_b_depth_numpy.shape
        image_width = image_b_shape[1]
        image_height = image_b_shape[0]

        blind_uv_a_flat = SD.flatten_uv_tensor(blind_uv_a, image_width, image_height)
        blind_uv_b_flat = SD.flatten_uv_tensor(blind_uv_b, image_width, image_height)
        ...
        return metadata[
                   "type"], image_a_rgb, image_b_rgb, empty_tensor, empty_tensor, empty_tensor, empty_tensor, empty_tensor, empty_tensor, blind_uv_a_flat, blind_uv_b_flat, metadata
    ...
    @staticmethod
    def flatten_uv_tensor(uv_tensor, image_width, image_height):
        """
        ...
        return torch.min(result, torch.ones_like(result).long() * ((image_width * image_height) - 1))
    ...
    def subsample_tuple_pair(uv_a, uv_b, num_samples):
        ...
        uv_a_downsampled = (
        torch.index_select(uv_a[0], 0, indexes_to_keep), torch.index_select(uv_a[1], 0, indexes_to_keep))
        uv_b_downsampled = (
        torch.index_select(uv_b[0], 0, indexes_to_keep), torch.index_select(uv_b[1], 0, indexes_to_keep))
        return uv_a_downsampled, uv_b_downsampled
    
    def mesh_distance(self, idx1, idx2, image_width, x_symmetry=False):
        knots = self.get_knots_info('shirt_canonical')['cam3-0-0']
        p1 = np.array(knots[idx1])
        p2 = np.array(knots[idx2])
        if not x_symmetry:
            return np.linalg.norm(p1-p2, ord=2)
        else:
            d1 = np.linalg.norm(p1-p2, ord=2)
            p2[0] = image_width - p2[0]
            d2 = np.linalg.norm(p1-p2, ord=2)
            return min(d1, d2)
        
    def closest_knot(self, knots, xyz):
        return min([np.linalg.norm(p - xyz, ord=2) for p in knots])
