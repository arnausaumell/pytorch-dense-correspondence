...
class DenseCorrespondenceDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        uv_b_non_matches = correspondence_finder.create_non_correspondences(uv_b, image_b_shape,
                                                                            num_non_matches_per_match=self.num_non_matches_per_match,
        ...
        if self.debug:
            ...
            if uv_a is not None:
                fig, axes = correspondence_plotter.plot_correspondences_direct(image_a_rgb, image_a_depth_numpy,
                                                                               image_b_rgb, image_b_depth_numpy, uv_a,
                                                                               uv_b, show=False)
                correspondence_plotter.plot_correspondences_direct(image_a_rgb, image_a_depth_numpy, image_b_rgb,
                                                                   image_b_depth_numpy,
    ...
    def get_rgbd_mask_pose(self, scene_name, img_idx):
        ...
        depth = self.get_depth_values(depth_file)

        mask_file = self.get_image_filename(scene_name, img_idx, ImageType.MASK)
        mask = self.get_mask_image(mask_file)

        pose = None  # self.get_pose_from_scene_name_and_idx(scene_name, img_idx)
    ...
    def get_depth_values(self, depth_filename):
        """
        :param depth_filename: string of full path to depth image
        :return: PIL.Image.Image
        """
        return cv2.imread(depth_filename, cv2.IMREAD_ANYCOLOR | cv2.IMREAD_ANYDEPTH)
    ...
    def init_length(self):
        ...
        for scene_name in self.scene_generator():
            scene_directory = self.get_full_path_for_scene(scene_name)
            rgb_images_regex = os.path.join(scene_directory, "images/rgb*.png")
    ...
    def set_parameters_from_training_config(self, training_config):
        ...
        p = float(training_config["training"]["data_type_probabilities"]["SINGLE_OBJECT_WITHIN_SCENE"])
        if p > 0:
