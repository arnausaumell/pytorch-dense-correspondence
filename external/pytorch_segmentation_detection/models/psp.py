...
class PSP_head(nn.Module):
    ...
    def forward(self, x):
        ...
        pooled_1 = nn.functional.interpolate(pooled_1, size=fcn_features_spatial_dim)

        pooled_2 = nn.functional.adaptive_avg_pool2d(x, 2)
        pooled_2 = self.conv2(pooled_2)
        pooled_2 = nn.functional.interpolate(pooled_2, size=fcn_features_spatial_dim)

        pooled_3 = nn.functional.adaptive_avg_pool2d(x, 3)
        pooled_3 = self.conv3(pooled_3)
        pooled_3 = nn.functional.interpolate(pooled_3, size=fcn_features_spatial_dim)

        pooled_4 = nn.functional.adaptive_avg_pool2d(x, 6)
        pooled_4 = self.conv4(pooled_4)
        pooled_4 = nn.functional.interpolate(pooled_4, size=fcn_features_spatial_dim)
...
class Resnet50_8s_psp(nn.Module):
    ...
    def forward(self, x):
        ...
        x = nn.functional.interpolate(input=x, size=input_spatial_dim)
