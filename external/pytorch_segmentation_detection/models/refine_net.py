class RefineNet(nn.Module):
    ...
    def forward(self, x):
        ...
        global_pool = nn.functional.interpolate(global_pool, size=layer4_output.size()[2:])
        layer_4_refined = self.layer_4_refine_right( self.layer_4_refine_left(layer4_output) + global_pool )
        
        
        layer_4_refined = nn.functional.interpolate(layer_4_refined, size=layer3_output.size()[2:])
        layer_3_refined = self.layer_3_refine_right( self.layer_3_refine_left(layer3_output) + layer_4_refined )
        
        layer_3_refined = nn.functional.interpolate(layer_3_refined, size=layer2_output.size()[2:])
        layer_2_refined = self.layer_2_refine_right( self.layer_2_refine_left(layer2_output) + layer_3_refined )
        
        layer_2_refined = nn.functional.interpolate(layer_2_refined, size=layer1_output.size()[2:])
        ...
        logits_upsampled = nn.functional.interpolate(logits,
